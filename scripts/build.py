...
def render_readme(tools_doc, routing_doc):
    ...
    L.append("")
    srcs = tools_doc.get("sources") or []
    if srcs:
        L.append("## Откуда это собрано")
        L.append("")
        L.append(
            "Часть инструментов вычёсана из чужих awesome-списков. Ссылки оттуда "
            "перепроверялись заново, описания написаны свои — прямого копирования нет. "
            "Спасибо их авторам:"
        )
        L.append("")
        for x in srcs:
            L.append("- [{}]({})".format(x["name"], x["url"]))
        L.append("")
        L.append(
            "Отличие этого каталога от них — не в объёме. Здесь каждая строка говорит, "
            "**зачем** брать инструмент и чем он отличается от соседа по таблице, "
            "а ссылки проверяются автоматически в CI."
        )
        L.append("")
