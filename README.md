...
**471 инструментов · 14 категорий · 58 подключаются к агенту по MCP · 263 бесплатны или open source**
...
| Подогнать съехавшие субтитры под звук | [ffsubsync](https://github.com/smacke/ffsubsync) · [stable-ts](https://github.com/jianfch/stable-ts) · [Aegisub](https://aegisub.org/) | Нужно каждый раз, когда видео перемонтировали после того, как субтитры уже сделали. |
| Расшифровать интервью с несколькими голосами | [Otter.ai](https://otter.ai/) · [Sonix](https://sonix.ai/) · [Trint](https://trint.com/) · [AssemblyAI](https://www.assemblyai.com/) · [pyannote.audio](https://github.com/pyannote/pyannote-audio) |  |
| Перевести и озвучить на другой язык | [Rask AI](https://www.rask.ai/) · [Dubverse](https://dubverse.ai/) · [HeyGen](https://www.heygen.com/) · [ElevenLabs](https://elevenlabs.io/) · [DeepL](https://www.deepl.com/) | Один ролик открывает несколько рынков — самый дешёвый рост из возможных. |
| Разложить готовое по площадкам | [Postiz](https://postiz.com/) · [Buffer](https://buffer.com/) · [Metricool](https://metricool.com/) · [Publer](https://publer.com/) · [Repurpose.io](https://repurpose.io/) | Postiz и Mixpost ставятся на свой сервер — единственные без подписки. |
...
| Задача | Чем делать | Комментарий |
|---|---|---|
| Где вообще запускать агента | [Claude Code](https://claude.com/product/claude-code) · [Claude Desktop](https://claude.ai/download) · [Cursor](https://cursor.com/) · [Cline](https://cline.bot/) · [Zed](https://zed.dev/) · [OpenCode](https://opencode.ai/) | Все понимают MCP — серверы из каталога подключаются к любому. |
| Найти готовый MCP-сервер под задачу | [Smithery](https://smithery.ai/) · [Glama MCP Directory](https://glama.ai/mcp/servers) · [MCP Registry](https://github.com/modelcontextprotocol/registry) · [Docker MCP Catalog](https://hub.docker.com/mcp) · [PulseMCP](https://www.pulsemcp.com/) | Glama показывает оценку качества и безопасности, а не просто список. |
| Написать свой MCP-сервер, если готового нет | [FastMCP](https://github.com/jlowin/fastmcp) · [MCP Inspector](https://github.com/modelcontextprotocol/inspector) | Inspector — первое, что запускаешь, когда сервер молчит. |
| Связать шаги в конвейер без кода | [n8n](https://n8n.io/) · [Activepieces](https://www.activepieces.com/) · [Make](https://www.make.com/) · [Windmill](https://www.windmill.dev/) · [Node-RED](https://nodered.org/) | n8n, Activepieces и Windmill ставятся на свой сервер. |
| Собрать команду агентов под роли | [CrewAI](https://www.crewai.com/) · [LangGraph](https://www.langchain.com/langgraph) · [AutoGen](https://microsoft.github.io/autogen/) · [Mastra](https://mastra.ai/) | Ложится на производство ролика: ресёрчер, сценарист, редактор. |
| Прогнать долгий рендер фоном, без таймаутов | [Trigger.dev](https://trigger.dev/) · [Windmill](https://www.windmill.dev/) · [Modal](https://modal.com/) · [RunPod](https://www.runpod.io/) |  |
| Проверить структуру ответа модели | [Pydantic AI](https://ai.pydantic.dev/) | Типизированный ответ не даст вернуть мусор вместо данных для сцены. |
| Понять, что агент делал и сколько потратил | [Langfuse](https://langfuse.com/) | Без трассировки длинный пайплайн отлаживается вслепую. |

## Категории

- 🤖 **[Агенты и оркестрация](#cat-agents)** — Среда, из которой всё запускается. Агент читает задачу и сам дёргает нужные инструменты. _(41)_
...
- ✂️ **[Переупаковка, субтитры, раскладка](#cat-repurpose)** — Выжать из снятого максимум: нарезать на короткие, вычистить речь, сделать субтитры, перевести и разложить по площадкам. Самый дешёвый контент — тот, что уже снят. _(38)_
...
| **[AutoGen](https://microsoft.github.io/autogen/)** | Каркас Microsoft для разговаривающих между собой агентов. Сильная сторона — проверка работы друг друга. | библиотека | 🔵 open source | `API` | — |
...
| **[Claude Desktop](https://claude.ai/download)** ⭐ | Настольное приложение с поддержкой MCP. Для тех, кому терминал не нужен, а Palmier и Blender подключить хочется. | приложение | 🟡 freemium | `MCP` | — |
| **[Cline](https://cline.bot/)** | Открытый агент внутри VS Code со своим ключом от любой модели. Умеет ставить MCP-серверы сам. | приложение | 🔵 open source | `MCP` | нужен |
...
| **[Continue](https://continue.dev/)** | Открытый ассистент для VS Code и JetBrains, работает и с локальными моделями через Ollama. | приложение | 🔵 open source | `MCP` | опц. |
| **[CrewAI](https://www.crewai.com/)** | Команда агентов с ролями и задачами. Ложится на производство ролика: сценарист, ресёрчер, редактор. | библиотека | 🔵 open source | `API` | — |
...
| **[Dify](https://dify.ai/)** | Визуальный конструктор AI-приложений, который ставится на свой сервер. Пайплайн собирается мышкой. | приложение | 🔵 open source | `API` | — |
| **[FastMCP](https://github.com/jlowin/fastmcp)** | Свой MCP-сервер на Python в несколько десятков строк. Так закрывают дыру, когда готового сервера нет. | библиотека | 🔵 open source | — | — |
| **[Flowise](https://flowiseai.com/)** | Нодовый редактор цепочек и агентов. Быстро проверить идею до того, как писать код. | приложение | 🔵 open source | `API` | — |
...
| **[Glama MCP Directory](https://glama.ai/mcp/servers)** | Каталог MCP-серверов с оценкой качества и безопасности. Полезнее голого списка на GitHub. | сервис | 🟢 бесплатно | — | — |
| **[Goose](https://block.github.io/goose/)** | Открытый агент от Block с расширениями поверх MCP. Работает и в терминале, и с интерфейсом. | приложение | 🔵 open source | `MCP` | нужен |
| **[Langflow](https://www.langflow.org/)** | Визуальная сборка потоков с экспортом в код. Мостик между прототипом на мышке и рабочим скриптом. | приложение | 🔵 open source | `API` | — |
| **[Langfuse](https://langfuse.com/)** | Видно, что агент реально делал, сколько потратил и где сломался. Без этого длинный пайплайн отлаживается вслепую. | приложение | 🔵 open source | `API` | опц. |
| **[LangGraph](https://www.langchain.com/langgraph)** | Агент как граф состояний с циклами и ветвлениями. Когда пайплайн должен уметь возвращаться на шаг назад. | библиотека | 🔵 open source | `API` | — |
| **[LlamaIndex](https://www.llamaindex.ai/)** | Подключает агента к своим документам и базам. Отсюда берётся ответ по архиву сценариев, а не по интернету. | библиотека | 🔵 open source | `API` | — |
...
| **[Mastra](https://mastra.ai/)** | Каркас агентов на TypeScript. Тот же язык, что у Remotion — пайплайн не разъезжается по двум экосистемам. | библиотека | 🔵 open source | `API` | — |
| **[MCP Inspector](https://github.com/modelcontextprotocol/inspector)** | Отладчик MCP-серверов: видно инструменты, аргументы и ответы. Первое, что запускаешь, когда сервер молчит. | приложение | 🔵 open source | — | — |
...
| **[Node-RED](https://nodered.org/)** | Проводной редактор потоков, живущий с 2013 года. Крутится хоть на Raspberry Pi рядом со студией. | приложение | 🔵 open source | `API` | — |
| **[OpenCode](https://opencode.ai/)** | Открытый терминальный агент, не привязанный к одному провайдеру моделей. | приложение | 🔵 open source | `MCP` | нужен |
| **[Pipedream](https://pipedream.com/)** | Интеграции со вставками своего кода между шагами. Гибче Zapier там, где нужна логика. | сервис | 🟡 freemium | `API` | нужен |
| **[PulseMCP](https://www.pulsemcp.com/)** | Новости и свежие MCP-серверы с рассылкой. Способ не пропустить появление нужного сервера. | сервис | 🟢 бесплатно | — | — |
| **[Pydantic AI](https://ai.pydantic.dev/)** | Агенты с типизированным ответом, который валидируется. Модель не может вернуть мусор вместо структуры. | библиотека | 🔵 open source | `API` | — |
| **[Roo Code](https://roocode.com/)** | Форк Cline с режимами под разные роли. Полезно, когда один агент пишет сценарий, а другой — код сцены. | приложение | 🔵 open source | `MCP` | нужен |
...
| **[Trigger.dev](https://trigger.dev/)** | Долгие фоновые задачи без таймаутов и с ретраями. Под рендер, который идёт полчаса. | сервис | 🔵 open source | `API` | опц. |
| **[Windmill](https://www.windmill.dev/)** | Свои скрипты на Python или TypeScript превращаются в задачи с расписанием и интерфейсом. Самохостится. | приложение | 🔵 open source | `API` | — |
| **[Windsurf](https://windsurf.com/)** | Редактор с агентом, конкурент Cursor. Тоже понимает MCP — серверы из каталога подключаются и сюда. | приложение | 🟡 freemium | `MCP` | — |
...
| **[Zed](https://zed.dev/)** | Очень быстрый открытый редактор на Rust с агентом и MCP. Заметно легче Electron-собратьев. | приложение | 🟡 freemium | `MCP` | опц. |
...
| Инструмент | Что делает | Тип | Цена | Из агента | Ключ |
|---|---|---|---|---|---|
| **[2short.ai](https://2short.ai/)** | Простая нарезка с бесплатным лимитом. Хорош, чтобы проверить формат до покупки подписки. | сервис | 🟡 freemium | — | — |
| **[Aegisub](https://aegisub.org/)** | Ветеран субтитров с покадровым таймингом и стилями. Когда нужен полный контроль, а не автомат. | приложение | 🔵 open source | — | — |
...
| **[Buffer](https://buffer.com/)** | Классический планировщик с щедрым бесплатным тарифом. Ставится за пять минут. | сервис | 🟡 freemium | `API` | нужен |
...
| **[DeepL](https://www.deepl.com/)** | Перевод, который заметно живее машинного. Для субтитров разница слышна сразу. | API | 🟡 freemium | `API` | нужен |
...
| **[Dubverse](https://dubverse.ai/)** | Дубляж с сильной поддержкой индийских языков и API. Ниша, которую западные сервисы закрывают плохо. | сервис | 🟡 freemium | `API` | нужен |
| **[ffsubsync](https://github.com/smacke/ffsubsync)** | Подгоняет съехавшие субтитры под звуковую дорожку автоматически. Спасает, когда видео перемонтировали. | CLI | 🔵 open source | `CLI` | — |
...
| **[Later](https://later.com/)** | Планировщик с визуальной сеткой ленты. Видно, как профиль будет выглядеть целиком. | сервис | 🟡 freemium | — | — |
| **[Metricool](https://metricool.com/)** | Планирование и аналитика в одном окне, включая TikTok и YouTube. Бесплатный тариф рабочий. | сервис | 🟡 freemium | `API` | нужен |
| **[Mixpost](https://mixpost.app/)** | Самохостимый планировщик с разовой лицензией. Данные аккаунтов остаются у тебя. | приложение | 🔵 open source | — | — |
| **[Munch](https://www.getmunch.com/)** | Режет на клипы, опираясь на данные о трендах, а не только на всплески речи. Объясняет, почему выбрал именно этот кусок. | сервис | 🔴 платно | — | — |
...
| **[Otter.ai](https://otter.ai/)** | Расшифровка встреч и интервью в реальном времени. Бесплатного лимита хватает на разовые задачи. | сервис | 🟡 freemium | — | — |
| **[Postiz](https://postiz.com/)** | Открытый планировщик постов, который ставится на свой сервер. Единственный в категории без подписки. | приложение | 🔵 open source | `API` | — |
| **[Publer](https://publer.com/)** | Много площадок и массовая загрузка расписания из таблицы. Удобно, когда постов десятки. | сервис | 🟡 freemium | — | — |
...
| **[Rask AI](https://www.rask.ai/)** | Дубляж на десятки языков с сохранением голоса и липсинком. Один ролик открывает несколько рынков. | сервис | 🔴 платно | — | — |
| **[Recut](https://getrecut.com/)** | Вырезает тишину локально и отдаёт готовый проект в Premiere, Resolve или Final Cut. Разовая покупка. | приложение | 🔴 платно | — | — |
| **[Repurpose.io](https://repurpose.io/)** | Автоматически перекидывает вышедший ролик на другие площадки с нужным форматом. Настроил один раз — работает само. | сервис | 🔴 платно | — | — |
...
| **[Sonix](https://sonix.ai/)** | Расшифровка и перевод в 50+ языков с редактором. Берут, когда важна точность на именах и терминах. | сервис | 🔴 платно | `API` | нужен |
| **[Spikes Studio](https://spikes.studio/)** | Нарезка со стилями субтитров и авто-кадрированием под лицо в кадре. | сервис | 🟡 freemium | — | — |
| **[stable-ts](https://github.com/jianfch/stable-ts)** | Делает тайминги Whisper устойчивыми и точными по словам. То, из чего собираются «прыгающие» субтитры. | библиотека | 🔵 open source | `CLI` | — |
...
| **[TimeBolt](https://www.timebolt.io/)** | Убирает паузы и слова-паразиты пачкой. Считает сэкономленные минуты — видно, окупился ли. | приложение | 🔴 платно | — | — |
| **[Trint](https://trint.com/)** | Расшифровка с редакторской вычиткой, заточенная под журналистику. Текст правится вместе со звуком. | сервис | 🔴 платно | `API` | нужен |
| **[VEED](https://www.veed.io/)** | Браузерный монтаж с субтитрами и переводом. Быстрая правка без установки чего-либо. | сервис | 🟡 freemium | — | — |
| **[vidyo.ai](https://vidyo.ai/)** | Клипы плюс готовые подписи под каждую площадку. Закрывает и нарезку, и текст к посту. | сервис | 🟡 freemium | — | — |
| **[Vizard](https://vizard.ai/)** | Нарезка на клипы с уклоном в многоязычные субтитры. | сервис | 🟡 freemium | — | — |
| **[Wisecut](https://www.wisecut.video/)** | Вырезает паузы и сам подкладывает музыку с автопригасом под речь. Два шага монтажа за один проход. | сервис | 🟡 freemium | — | — |
| **[Zubtitle](https://zubtitle.com/)** | Субтитры, ресайз и заголовок в кадре одним прогоном. Заточен под соцсети. | сервис | 🔴 платно | — | — |
